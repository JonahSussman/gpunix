...
/* 
Running Unix on CUDA is entirely impractical for a number of reasons.

https://stackoverflow.com/questions/34989481/how-to-interrupt-or-cancel-a-cuda-kernel-from-host-code
*/
...
int main(void) {
  ...
  cudaError_t err_set_limit = cudaDeviceSetLimit(cudaLimitStackSize, CUDA_LIMIT_STACK_SIZE);
  if (err_set_limit != cudaSuccess) {
    printf("Error setting stack size: %s\n", cudaGetErrorString(err_set_limit));
    return -1;
  }

  size_t* stack_size = new size_t;
  cudaError_t err_get_limit = cudaDeviceGetLimit(stack_size, cudaLimitStackSize);

  if (err_get_limit != cudaSuccess) {
    printf("Error getting stack size: %s\n", cudaGetErrorString(err_get_limit));
    return -1;
  }
