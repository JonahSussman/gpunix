...
#define MINIRV32_HANDLE_MEM_STORE_CONTROL(state, addy, val)                                        \
...
__global__ void MiniRV32IMAStepKernel(
  ...
  *ret = MiniRV32IMAStep(state, image, vProcAddress, elapsedUs, count);
}
...
__device__ int IsKBHit() { return d_kb_state->hit; }
